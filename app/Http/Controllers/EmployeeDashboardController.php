...
class EmployeeDashboardController extends Controller
{
    ...
    public function scheduleEvents(Request $request)
    {
        ...
        $overtimeSchedules = EmployeeOvertimeSchedule::where('employee_id', $employeeId)
            ->where('status', 'approved')
            ->whereBetween('ot_date', [$start->toDateString(), $end->toDateString()])
            ->get()
            ->groupBy(function ($row) {
                return Carbon::parse($row->ot_date)->toDateString();
            });
        ...
        for ($date = $start->copy(); $date->lte($end); $date->addDay()) {
            ...
            if (isset($overtimeSchedules[$d])) {
                foreach ($overtimeSchedules[$d] as $ot) {
                    $otStart = Carbon::parse($d . ' ' . $ot->start_time, 'Asia/Manila');
                    $otEnd   = Carbon::parse($d . ' ' . $ot->end_time, 'Asia/Manila');
                    ...
                    $events[] = [
                        ...
                        'start' => $otStart->toIso8601String(),
                        'end'   => $otEnd->toIso8601String(),
